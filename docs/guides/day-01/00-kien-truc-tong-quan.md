...
- **Domain** là trái tim nghiệp vụ: entity (vd `User`, `RefreshToken`), value object, luật nghiệp vụ thuần. **Không tham chiếu gì cả**: không EF Core, không ASP.NET. Nhờ vậy nó dễ test nhất.
...
4. **`Contracts`** chứa integration event (vd `TicketSoldEvent`) và là bề mặt công khai *duy nhất* giữa các module qua message bus. `SharedKernel` chỉ là viên gạch dùng chung *nội bộ* (`Result<T>`, base types, guards), **không** phải kênh giao tiếp cross-module.
5. Không để làm gì cả. Đây là **file rỗng do template `dotnet new classlib` tự sinh**. Chỉ là rác mẫu, cần xóa.
