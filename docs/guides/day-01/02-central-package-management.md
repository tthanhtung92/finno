...
**Central Package Management (CPM)** sửa tận gốc: version khai **một nơi** (`Directory.Packages.props` ở gốc), các `.csproj` chỉ nói "tôi _cần_ package này", không kèm số. Một chỗ sửa, cả solution theo.
